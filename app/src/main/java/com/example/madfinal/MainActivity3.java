...
public class MainActivity3 extends AppCompatActivity implements AdapterView.OnItemSelectedListener  {
    ...
    String[] courses = { "Enter Hospital Id" ,
            "KMC mangalore", "AJ Hospital",
            "Vinay Hospital", "TMA pai Hospital"};
    ...
    public void onItemSelected(AdapterView<?> adapterView, View view, int position, long id) {
        ...
        // Handle the selection for each spinner independently
        switch (spinnerId) {
            case R.id.spinner:  // Hospital spinner
                Toast.makeText(getApplicationContext(),
                        courses[position],
                        Toast.LENGTH_LONG).show();
                break;

            case R.id.spinner1:  // Blood group spinner
                Toast.makeText(getApplicationContext(),
                        courses1[position],
                        Toast.LENGTH_LONG).show();
                // Handle the case for "O-" selection properly
                if (position == courses1.length - 1) {
                    // Do something if needed when "O-" is selected
                }
                break;

            case R.id.spinner2:  // Organ spinner
                Toast.makeText(getApplicationContext(),
                        courses2[position],
                        Toast.LENGTH_LONG).show();
                break;
